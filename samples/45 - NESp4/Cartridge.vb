Public Class Cartridge
  ...
  Public Function CpuRead(addr As UShort, ByRef data As Byte) As Boolean
    Dim mappedAddr As UInteger = 0
    If m_mapper.CpuMapRead(addr, mappedAddr) Then
      data = m_prgMemory(CInt(mappedAddr))
      Return True
    Else
      Return False
    End If
  ...
  Public Function CpuWrite(addr As UShort, data As Byte) As Boolean
    Dim mappedAddr As UInteger = 0
    If m_mapper.CpuMapWrite(addr, mappedAddr, data) Then
      m_prgMemory(CInt(mappedAddr)) = data
      Return True
    Else
      Return False
    End If
  ...
  Public Function PpuRead(addr As UShort, ByRef data As Byte) As Boolean
    Dim mappedAddr As UInteger = 0
    If m_mapper.PpuMapRead(addr, mappedAddr) Then
      data = m_chrMemory(CInt(mappedAddr))
      Return True
    Else
      Return False
    End If
  ...
  Public Function PpuWrite(addr As UShort, data As Byte) As Boolean
    Dim mappedAddr As UInteger = 0
    If m_mapper.PpuMapWrite(addr, mappedAddr) Then
      m_chrMemory(CInt(mappedAddr)) = data
      Return True
    Else
      Return False
    End If
